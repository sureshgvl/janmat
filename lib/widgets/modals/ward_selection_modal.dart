...
class _WardSelectionModalState extends State<WardSelectionModal> {
  ...
  void _showAreasDialog(BuildContext context, Ward ward) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Areas in ${ward.name}'),
          content: SizedBox(
            width: double.maxFinite,
            child: ListView.builder(
              shrinkWrap: true,
              itemCount: ward.areas?.length ?? 0,
              itemBuilder: (context, index) {
                return ListTile(
                  title: Text(ward.areas![index]),
                );
              },
            ),
          ),
  ...
  void _showWardAreasModal(BuildContext context) {
    // Sort wards by number
    final sortedWards = List<Ward>.from(widget.wards);
    sortedWards.sort((a, b) {
      final aNumber = int.tryParse(RegExp(r'ward_(\d+)').firstMatch(a.id.toLowerCase())?.group(1) ?? '0') ?? 0;
      final bNumber = int.tryParse(RegExp(r'ward_(\d+)').firstMatch(b.id.toLowerCase())?.group(1) ?? '0') ?? 0;
      return aNumber.compareTo(bNumber);
    });
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        ProfileLocalizations.current.wardAreasTitle,
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          // Ward Name with Gray Background
                          Container(
                            ...
                            child: Text(
                              _formatWardDisplay(ward.id, ward.name),
                              style: const TextStyle(
                                fontSize: 18,
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        ProfileLocalizations.current.selectYourWard,
                        style: const TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                          color: Colors.purple,
                ...
                // Area View Button
                ElevatedButton.icon(
                  onPressed: () => _showWardAreasModal(context),
                  icon: const Icon(Icons.list),
                  label: Text(ProfileLocalizations.current.viewWardAreas),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: ProfileLocalizations.current.searchWards,
