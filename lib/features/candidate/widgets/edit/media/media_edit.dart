...
class MediaTabEditState extends State<MediaTabEdit> {
  ...
  Future<void> _saveMedia() async {
    ...
    try {
      ...
      // Convert MediaItem list to List<Media> for the controller
      final mediaList = _mediaItems.map((item) {
        // Convert MediaItem to Media objects
        final allMedia = <Media>[];
        // Add images as Media objects
        ...
        // Add videos as Media objects
        for (final videoUrl in item.videos) {
          allMedia.add(Media(
            url: videoUrl,
            title: item.title,
            description: item.title,
        ...
        // Add YouTube links as Media objects
        for (final youtubeUrl in item.youtubeLinks) {
          allMedia.add(Media(
            url: youtubeUrl,
            title: item.title,
            description: item.title,
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to save media'),
            backgroundColor: Colors.red,
          ),
        );
      }
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: Text(_isSaving ? 'Saving...' : 'Save Media'),
