...
abstract class IAchievementsController {
  ...
  Future<bool> saveAchievements(
    Candidate candidate,
    AchievementsModel achievements,
  );
  Future<bool> updateAchievementsFields(
    Candidate candidate,
  ...
  AchievementsModel getUpdatedCandidate(
    AchievementsModel current,
    String field,
    dynamic value,
  );
}
...
class AchievementsController extends GetxController
    implements IAchievementsController {
  ...
  Future<AchievementsModel?> getAchievements(Candidate candidate) async {
    try {
      AppLogger.database(
        'AchievementsController: Fetching achievements for ${candidate.candidateId}',
        tag: 'ACHIEVEMENTS_CTRL',
      );
      return await _repository.getAchievements(candidate);
    } catch (e) {
      AppLogger.databaseError(
        'AchievementsController: Error fetching achievements',
        tag: 'ACHIEVEMENTS_CTRL',
        error: e,
      );
  ...
  Future<bool> saveAchievements(
    ...
    try {
      AppLogger.database(
        'AchievementsController: Saving achievements for ${candidate.candidateId}',
        tag: 'ACHIEVEMENTS_CTRL',
      );
      return await _repository.updateAchievements(
      ...
      AppLogger.databaseError(
        'AchievementsController: Error saving achievements',
        tag: 'ACHIEVEMENTS_CTRL',
        error: e,
      );
  ...
  Future<bool> updateAchievementsFields(
    ...
    try {
      AppLogger.database(
        'AchievementsController: Updating achievements fields for ${candidate.candidateId}',
        tag: 'ACHIEVEMENTS_CTRL',
      );
      return await _repository.updateAchievementsFields(candidate, updates);
    } catch (e) {
      AppLogger.databaseError(
        'AchievementsController: Error updating achievements fields',
        tag: 'ACHIEVEMENTS_CTRL',
        error: e,
      );
  ...
  AchievementsModel getUpdatedCandidate(
    ...
    AppLogger.database(
      'AchievementsController: Updating field $field with value $value',
      tag: 'ACHIEVEMENTS_CTRL',
    );
    ...
    switch (field) {
      case 'achievements':
        return current.copyWith(
          achievements: value is List
              ? List<Achievement>.from(
                  value.map(
                    (a) => a is Achievement ? a : Achievement.fromJson(a),
                  ),
                )
      ...
      default:
        AppLogger.database(
          'AchievementsController: Unknown field $field, returning unchanged',
          tag: 'ACHIEVEMENTS_CTRL',
        );
        return current;
    }
  ...
  Future<bool> saveAchievementsTab({
    ...
    try {
      AppLogger.database(
        '🏆 TAB SAVE: Achievements tab for $candidateId',
        tag: 'ACHIEVEMENTS_TAB',
      );

      onProgress?.call('Saving achievements...');

      // Direct save using the repository
      final success = await _repository.updateAchievements(
        candidateId,
        achievements,
        candidate,
      );
      ...
      if (success) {
        ...
        AppLogger.database(
          '✅ TAB SAVE: Achievements completed successfully',
          tag: 'ACHIEVEMENTS_TAB',
        );
        return true;
      } else {
        AppLogger.databaseError(
          '❌ TAB SAVE: Achievements save failed',
          tag: 'ACHIEVEMENTS_TAB',
        );
        return false;
      }
      ...
      AppLogger.databaseError(
        '❌ TAB SAVE: Achievements tab save failed',
        tag: 'ACHIEVEMENTS_TAB',
        error: e,
      );
      return false;
  ...
  Future<bool> saveAchievementsFast(
    ...
    try {
      AppLogger.database(
        '🚀 FAST SAVE: Achievements for $candidateId',
        tag: 'ACHIEVEMENTS_FAST',
      );
  ...
  void _runBackgroundSyncOperations(
    ...
    try {
      ...
      backgroundOperations.add(
        _sendAchievementsUpdateNotification(candidateId, updates),
      );
  ...
  Future<void> _sendAchievementsUpdateNotification(
    String candidateId,
    Map<String, dynamic> updates,
  ) async {
    try {
      ...
      AppLogger.database(
        '🔔 BACKGROUND: Achievements notification sent',
        tag: 'ACHIEVEMENTS_FAST',
      );
    } catch (e) {
      AppLogger.databaseError(
